...
public class Elios : Player
{
    ...
    public override async Task UseSkill()
    {
        ...
        IncreaseAttackPower(_skillData.ValueIndex); //공격력 상승
        var returnAttackPower = new TurnEffect
        {
            Duration = 1,
            OnTurnEnd = () => DecreaseAttackPower(_skillData.ValueIndex) //공격력 저하
        };
        AddTurnEffect(returnAttackPower);
    }
}
